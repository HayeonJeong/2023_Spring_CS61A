...
def pick(paragraphs, select, k):
    ...
    if len(true_list) - 1 < k:
        return ''
    else:
        return true_list[k]
    # END PROBLEM 1
    '''
    if select(paragraphs) == True:
        if k < len(paragraphs):
            k_th = paragraphs[k]
            del paragraphs[k]
            return k_th
...
def about(subject):
    ...
    def select(paragraphs):
        newList = remove_punctuation(paragraphs).lower().split()
        for word in subject:
            for p in newList:
                if word == p:
                    return True
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    accuracy_n = 0.0
    short_len = [len(typed_words), len(source_words)]
    for i in range(min(short_len)):
        if typed_words[i] == source_words[i]:
            accuracy_n += 100 / len(typed_words)  
...
def wpm(typed, elapsed):
    ...
    time_min = len(typed) / 5 * 60 #unit: min
    n_words_per_min = time_min / elapsed
    return n_words_per_min
