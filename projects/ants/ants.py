...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if self.lower_bound <= spot <= self.upper_bound:
                if place.bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            place = self.place
            if place.bees:
                target = random.choice(place.bees)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a (return True/False)
        "*** YOUR CODE HERE ***"
        self.other = other
...
# END Problem 12
    ...
    def remove_from(self, place):
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        #A queen cannot be removed.
        #Attempts to remove a queen should have no effect
        
        if type(self) is QueenAnt:
            return
